...
class Frame2D:
    def __init__(self):
        self.mat = np.mat([[1., 0., 0.], [0., 1., 0.], [0., 0., 1.]])

    @classmethod
    def fromMat(cls, m: np.mat):
        f = cls()
        f.mat = m
        return f
    ...
    def fromPose(cls, pose: Pose):
        ...
        f.mat = np.matrix(
            [[math.cos(angle), -math.sin(angle), pose.position.x], [math.sin(angle), math.cos(angle), pose.position.y],
             [0., 0., 1.]])
        return f;
    '''
    ...
    def fromXYA(cls, x, y=None, a=None):
        if y is not None:
            f = cls()
            f.mat = np.mat([[math.cos(a), -math.sin(a), x], [math.sin(a), math.cos(a), y], [0., 0., 1.]])
            return f
        else:
            f = cls()
            xm = x.reshape(-1)
            f.mat = np.mat([[math.cos(xm[2]), -math.sin(xm[2]), xm[0]], [math.sin(xm[2]), math.cos(xm[2]), xm[1]], [0., 0., 1.]])
            return f

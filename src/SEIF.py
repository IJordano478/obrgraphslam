    ...
    if 1 not in np.multiply(jBlanket, mBlanket):
        ...
        for lmark in path:
            ...
            for i in range(0, omega.omegaMatrix.shape[0] - 3, 2):
                if not np.allclose(omega.omegaMatrix[3 + lmark * 2:5 + lmark * 2, 3 + i:5 + i], np.zeros((2, 2))):
...
def plot_graph_from_omega(omega, xi, active=False, connections=False, holdprogram=True):
...
if __name__ == "__main__":
    ...
    # # print("Mean:\n", mean.round(3), "\n")
    #
    # plot_graph_from_omega(omega, xi, active=True, connections=True, holdProgram=False)
    # for m1 in range(0, (omega.omegaMatrix.shape[0]-3)//2):
    #     for m2 in range(0, (omega.omegaMatrix.shape[0] - 3) // 2):
    #         if m1 != m2:
    #             print("Comparing", m1, "to", m2, ":", seif_correspondence_test(omega, xi, mean, m1, m2))
